      ...
      db.deleteProfile(req.session.userId)
        .then(() => {
          db.deleteUser(req.session.userId)
            ...
            .catch((err) => {
              console.log("err in /deleteaccount deleting profile: ", err);
            });
        })
        .catch((err) => {
          console.log("err in /deleteaccount deleting profile: ", err);
      ...
      .then(() => {
        if (password) {
            ...
            db.updateFirstLastPasswordEmail(
              ...
              .then(() => {
                db.addOrUpdateAgeCityHomepage(
                  ...
                  .catch((err) => {
                    console.log("err in updating city and password: ", err);
                    res.redirect("/edit");
                  });
                //
              })
          ...
          db.updateFirstLastNoPasswordEmail(
            ...
            .then(() => {
              db.addOrUpdateAgeCityHomepage(age, city, url, req.session.userId)
                .then(() => {
                  res.redirect("/thanks");
                })
                .catch((err) => {
            ...
            .catch((err) => {
              console.log("err in updateFirstLastPasswordEmail: ", err);
              db.getAllForEdit(req.session.userId)
                .then((allInfo) => {
                  let age = allInfo.rows[0].age;
                  let city = allInfo.rows[0].city;
